...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	chain, _ := GenerateChain(gspec.Config, genesis, eccpow.NewFaker(), db, b.N, gen)

	// Time the insertion of the new chain.
	// State and blocks are stored in the same DB.
	chainman, _ := NewBlockChain(db, nil, gspec.Config, eccpow.NewFaker(), vm.Config{}, nil)
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
func benchReadChain(b *testing.B, full bool, count uint64) {
	...
	for i := 0; i < b.N; i++ {
		...
		chain, err := NewBlockChain(db, nil, params.TestChainConfig, eccpow.NewFaker(), vm.Config{}, nil)
		if err != nil {
